...
object Preferences {
    ...
    const val USB_DAC_EXCLUSIVE_ENABLED = "usb_dac_exclusive_enabled"
    ...
    @JvmStatic
    fun setEqualizerBandWeights(weights: FloatArray) {
        val asString = weights.joinToString(",")
        App.getInstance().preferences.edit().putString(EQUALIZER_BAND_WEIGHTS, asString).apply()
    }

    @JvmStatic
    fun getEqualizerBandWeights(bandCount: Short): FloatArray {
        val str = App.getInstance().preferences.getString(EQUALIZER_BAND_WEIGHTS, null)
        if (str.isNullOrBlank()) {
            return DEFAULT_BAND_WEIGHTS.clone()
        }
        val parts = str.split(",")
        if (parts.size < bandCount) return DEFAULT_BAND_WEIGHTS.clone()
        return FloatArray(bandCount.toInt()) { i -> parts[i].toFloatOrNull() ?: DEFAULT_BAND_WEIGHTS.getOrElse(i) { 0.5f } }
    }

    @JvmStatic
    fun setEqualizerMaxAttenuation(attenDb: Float) {
        App.getInstance().preferences.edit().putFloat(EQUALIZER_MAX_ATTENUATION, attenDb).apply()
    }

    @JvmStatic
    fun getEqualizerMaxAttenuation(): Float {
        return App.getInstance().preferences.getFloat(EQUALIZER_MAX_ATTENUATION, DEFAULT_MAX_ATTENUATION)
    }

    @JvmStatic
    fun setEqualizerSoftKneeThreshold(threshold: Float) {
        App.getInstance().preferences.edit().putFloat(EQUALIZER_SOFT_KNEE_THRESHOLD, threshold).apply()
    }

    @JvmStatic
    fun getEqualizerSoftKneeThreshold(): Float {
    ...
    @JvmStatic
    fun getEqualizerManualPreampDb(): Float {
        return App.getInstance().preferences.getFloat(EQUALIZER_MANUAL_PREAMP_DB, DEFAULT_MANUAL_PREAMP_DB)
    }
